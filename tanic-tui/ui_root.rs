...
use crate::{NamespaceListView, TableListView};

pub(crate) struct UiRoot<'a> {
...
impl UiRoot {
    pub(crate) fn new(state: &TanicAppState) -> Self {
        Self {
            state,
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        match &self.state {
            TanicAppState::Initializing => {
                render_view_initializing(top, buf)
            }
            TanicAppState::ViewingNamespacesList(_) => {
                self.namespace_list_view.render(top, buf)
            }
            TanicAppState::ViewingTablesList(_) => {
                self.table_list_view.render(top, buf)
            }
